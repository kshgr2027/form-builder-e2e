...
| CI/CD | ✅ `.github/workflows/ci.yml` written + reviewed — ⏳ not yet run end-to-end (needs the app at `./app` or `APP_REPO`, + a push) |
| Evidence capture (screenshots / traces per defect) | ⏳ In progress — `evidence/` structure |
| Git repo + push + Drive upload | ⏳ Pending push (Git now installed) |
